...
Библиотеку можно использовать как ядро валидации бизнес-правил и проверок данных в кредитном скоринге, кредитном конвейере, платёжном шлюзе, KYC и других банковских процессах. Расчётные сервисы передают в движок готовые факты: сумму, доход, долговую нагрузку, скоринговые показатели, баланс, лимиты, комиссию, валюту и состояние операции, а `jsonspecs` применяет к ним версионированные правила, проверяет форматы, диапазоны, взаимосвязи и условия принятия решения. После этого `jsonspecs` возвращает однозначный результат проверки, список структурированных ошибок и сведения о том, какой набор и какая версия правил использовались. Отдельный пакет операторов `@jsonspecs/decimal` позволяет безопасно проверять денежные значения без потери точности, а `@jsonspecs/operators-ru` добавляет проверки российских идентификаторов, документов и банковских реквизитов. Благодаря этому бизнес-политика хранится отдельно от кода сервисов, проходит ревью и тестирование как самостоятельный JSON-артефакт и может обновляться контролируемо без переноса расчётов или состояния внутрь движка.
